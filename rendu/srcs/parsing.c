...
/*   Updated: 2023/01/04 19:04:12 by acardona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/push_swap.h"

static int	fts_check_int_format(char *str)
{
	int	i;
	int	j;
...
static int	fts_parse_to_table(int ac, char **av, t_2int	**tab)
{
	...
	*tab = malloc((ac - 1) * sizeof(t_2int));
	...
	while (++i < ac - 1)
	{
		if (fts_check_int_format(av[i +1])
			|| (ft_atoi(av[i + 1]) == 0 && fts_check_atoi(av[i + 1])))
		{
			free(*tab);
			return (1);
		}

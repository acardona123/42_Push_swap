...
/*   Updated: 2022/12/27 23:47:50 by acardona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fts_piles_rm_op(t_piles_state piles, int op_id)
{
	...
	if (op_id < 4 || op_id > 7)
	{
		if (!(piles.opa))
		{
			ft_printf("Operation %d demandee sur une liste vide.\n", op_id);
			return ;
		}
		temp = piles.opa;
...
static void	fts_piles_do_one_operation(t_piles_state piles, int op_id)
{
	if (op_id == 0)
		ft_circlst_push(&(piles.pa), &(piles.pb));
	else if (op_id == 1)
		ft_circlst_swap(&(piles.pa));
	else if (op_id == 2)
		ft_circlst_rotate(&(piles.pa));
	else if (op_id == 3)
		ft_circlst_rev_rotate(&(piles.pb));
	else if (op_id == 4)
		ft_circlst_push(&(piles.pb), &(piles.pa));
	else if (op_id == 5)
		ft_circlst_swap(&(piles.pb));
	else if (op_id == 6)
		ft_circlst_rotate(&(piles.pb));
	else if (op_id == 7)
		ft_circlst_rev_rotate(&(piles.pb));
	else if (op_id == 8)
...
/*MAIN FONCTION : Activated when a push operation is orderd or at the end
...
int	ft_piles_do_all_operations(t_piles_state piles)
{
	...
	{
		if ((piles.opa)->op == (piles.opb)->op + 4)
			fts_piles_do_one_operation(piles, (piles.opb)->op + 3);
		else
		{
			...
			if (len_match_a < len_match_b)
				fts_piles_do_one_operation(piles, (piles.opa)->op);
			else
				fts_piles_do_one_operation(piles, (piles.opb)->op);
		}
	}
	while (piles.opa)
		fts_piles_do_one_operation(piles, (piles.opa)->op);
